...
def make_data_loader(cfg, is_train=True, distributed=False, max_iter=None, start_iter=0,adain=False):
    #if cfg.DATASETS.ADAIN_STYLE:  
    #  train_transform = build_transforms(cfg, is_train=False)
    #  print("make_data_loader con la configurazione ADAIN")
    #else:
    if adain:
      train_transform = build_transforms(cfg, is_train=is_train,is_adain_style=False)
    else:
      train_transform = build_transforms(cfg, is_train=is_train)    
    target_transform = build_target_transform(cfg) if is_train else None
    #dataset_list = cfg.DATASETS.ADAIN_STYLE + cfg.DATASETS.TRAIN if is_train else cfg.DATASETS.TEST
    dataset_list = cfg.DATASETS.TRAIN if is_train else cfg.DATASETS.TEST
    #dataset_list.append(cfg.DATASETS.ADAIN)
    print("DATASET LIST IN MAKE DATA LOADER "+ str(dataset_list))
    ...
    print("dataloaders generati:" + str(len(data_loaders)))
...
def make_style_data_loader(cfg, is_train=True, distributed=False, max_iter=None, start_iter=0):
    train_transform = build_transforms(cfg, is_train=False, is_adain_style=False) #E' necessario applicare le transform al dataset Clipart? Verificare questa transform
    #target_transform = build_target_transform(cfg) if is_train else None
    target_transform = None
    #dataset_list = cfg.DATASETS.ADAIN_STYLE + cfg.DATASETS.TRAIN if is_train else cfg.DATASETS.TEST
    dataset_list = cfg.DATASETS.ADAIN_STYLE
    #dataset_list.append(cfg.DATASETS.ADAIN)
    print("DATASET LIST IN MAKE STYLE DATA LOADER "+ str(dataset_list))
    datasets = build_dataset(dataset_list, transform=train_transform, target_transform=target_transform, is_train=is_train)
    ...
    for dataset in datasets:
        ...
        #batch_size = cfg.SOLVER.BATCH_SIZE #if is_train else cfg.TEST.BATCH_SIZE
        #batch_size=10
        batch_size = 5 #batch size is fixed to 5 due to GPU memory
        batch_sampler = torch.utils.data.sampler.BatchSampler(sampler=sampler, batch_size=batch_size, drop_last=True)

        data_loader = DataLoader(dataset, num_workers=cfg.DATA_LOADER.NUM_WORKERS, batch_sampler=batch_sampler,
                                 pin_memory=cfg.DATA_LOADER.PIN_MEMORY, collate_fn=BatchCollator(is_train=False))
        data_loaders.append(data_loader)
